...
namespace
{
constexpr GlyphCorner unitQuad[] = {
    {{0.f, 0.f}},
    {{1.f, 0.f}},
    {{0.f, 1.f}},
    {{1.f, 0.f}},
...
struct GlyphBatch::Program final : ShaderProgram
{
    ...
    {
        auto corner = vertexInput(&GlyphCorner::corner);
...
void GlyphBatch::begin()
{
    masks.clear();
    colors.clear();
}

void GlyphBatch::add(const Graphics::Rect& destination,
...
void GlyphBatch::drawQueue(RenderPass& pass,
...
void GlyphBatch::flush(RenderPass& pass, Text::GlyphAtlas& atlas)
{
    if (!prepared)
    {
